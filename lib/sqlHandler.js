...
SqlStore.prototype.update = function(request, partialResource, finishedCallback) {
  ...
  self.baseModel.findAll({
    ...
    return theResource.update(partialResource).then(function() {
      //
      // var tasks = { };
      // Object.keys(self.relations).forEach(function(relationName) {
      ...
      //           }, function(e) { console.error("ERR", e); return acallback(e); });
      //         }, taskCallback);
      //       });
      //     } else {
      //       relationModel.create(prop).then(function(i) {
      //         theResource["set" + uc](i);
      //         return taskCallback(null, i);
      //       }, function(e) { console.error("ERR", e); return taskCallback(e); });
      //     }
      //   };
      ...
      // async.parallel(tasks, function(err) {
      //   if (err) return finishedCallback(err);
      //   theResource.save().then(function() {
      //     return finishedCallback(null, null);
      //   }, function(finalErr) {
      //     return finishedCallback(finalErr);
      //   });
      // });
    }).then(function() { finishedCallback(null, self._fixObject(theResource.toJSON())); });
  }).catch(function(e) {
    return self._errorHandler(e, finishedCallback);
  });
};
