...
SqlStore.prototype._buildModels = function() {
  ...
  self.relations = Object.keys(relations).map(function(relationName) {
    var relation = relations[relationName];
    return self._defineRelationModel(relationName, relation._settings.__many);
  });
};
...
SqlStore.prototype._defineRelationModel = function(relationName, many) {
  ...
  if (many) {
    ...
    self.baseModel.hasMany(modelName);
  } else {
    modelProperties = _.extend(modelProperties, {
      relationId: {
        type: new Sequelize.STRING(38),
    ...
    self.baseModel.hasOne(modelName);
  }
