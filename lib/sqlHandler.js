...
SqlStore.prototype.create = function(request, newResource, finishedCallback) {
  ...
  breakPromise(self.sequelize.transaction(), function(err1, transaction) {
    ...
    breakPromise(self.baseModel.create(newResource), function(err2, baseInstance) {
        ...
        tasks[relationName] = function(taskCallback) {
          if (prop instanceof Array) {
            async.map(prop, function(item, callback) {
              breakPromise(relationModel.create(item), function(err, newRelationModel) {
            ...
            breakPromise(relationModel.create(prop), function(err, newRelationModel) {
    ...
    breakPromise(self.baseModel.findAll({
      ...
      Object.keys(self.relations).forEach(function(relationName) {
        ...
        tasks[relationName] = function(taskCallback) {
          if (prop instanceof Array) {
            ...
            breakPromise(theResource["set" + uc]([]), function(err3) {
              if (err3) return taskCallback(err3);

              async.map(prop, function(item, acallback) {
                breakPromise(relationModel.create(item), function(err4, newRelationModel) {
                  if (err4) return acallback(err4);

                  theResource["add" + uc](newRelationModel);
                  return acallback(null, newRelationModel);
                });
              }, taskCallback);
            });
          } else {
            ...
            if (!prop) {
              breakPromise(theResource["set" + uc](null), function(err3) {
                return taskCallback(err3);
              });
            } else {
              breakPromise(relationModel.create(prop), function(err3, newRelationModel) {
                if (err3) return taskCallback(err3);

                breakPromise(theResource["set" + uc](newRelationModel), function(err4) {
                  if (err4) return taskCallback(err4);
                  return taskCallback(null, newRelationModel);
                });
              });
            }
      ...
      if (Object.keys(_.omit(partialResource, self.relations)).length > 2) {
        tasks.__base__ = function(taskCallback) {
          breakPromise(theResource.update(partialResource), function(err) {
            if (err && (err.message !== "ER_EMPTY_QUERY: Query was empty")) return taskCallback(null, theResource);
